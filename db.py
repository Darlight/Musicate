...
while (opcion != 29):
    ...
    # Agregar usuario
        ...
        cur.execute(
            "INSERT INTO Users(userid, name, lastname, email, username, password, roleid) VALUES (%s, %s, %s, %s, %s, %s, %s)",
            (newid, name, lastname, email, user, password, roleid))
        con.commit()
    ...
    # Buscar usuario
        ...
        user = input("Ingrese el nombre del usuario: ")
        cur.execute("SELECT u.userid, u.username, u.email, u.password, u.role FROM Users u WHERE u.username = %s",
                    (user,))
        opcion1 = cur.fetchall()
        for r in opcion1:
    ...
    # Agregar cancion
        ...
        cur.execute(
            ...
            (trackid, name, albumid, mediatypeid, genreid, composer, milliseconds, size, unitprice))
        con.commit()
    ...
    if (opcion == 21):
        ...
        cur.execute(
            "UPDATE Track SET name = %s, albumid=%s, mediatypeid=%s, genreid=%s, composer=%s, milliseconds=%s, bytes=%s, unitprice=%s WHERE trackid=%s",
            (newname, albumid, mediatypeid, genreid, composer, milliseconds, size, unitprice, trackid,))
        con.commit()
